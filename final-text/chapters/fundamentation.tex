...
executadas em paralelo. Este insere uma camada intermediária entre 
hardware e o software desenvolvido que facilita a implementação pois provê uma 
certa independência de arquitetura para este.
...
BSP é quem desenvolve a aplicação, neste é quem desenvolve o ambiente 
de execução (\textit{runtime}). 
...
dos ambientes de execução de execução de aplicações baseadas em 
tarefas, como OmpSs, 
...
grandes volumes de dados. A principal motivação de seu desenvolvimento é o  
...
de recursos do arcabouço. O YARN (Yet Another Resource Negotiator) foi 
...
De forma similar a sistemas de arquivos convencionais, existe é utilizada a 
...
transformações.Isso permite que ele encontre um plano de execução eficiente pois 
pode agregar operações, otimizando o tempo de execução.
