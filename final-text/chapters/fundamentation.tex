...
Como entradas, são utilizados a representação do DAG e os \textit{traces} de 
execução. Essa ferramenta é desenvolvida especificamente para o 
ambiente de execução PaRSEC.

\subsubsection*{Temanejo}

Temanejo \cite{ref:temanejo} é um \textit{debugger} para o modelo de 
...
de execução. Ela divide os estados dos \textit{workers} em três categorias, 
permitindo a 
identificação de delays decorrentes de problemas de escalonamento. Como em 
ambientes heterogêneos com tarefas variadas, a presença de \textit{delays} faz 
...
com uma abordagem de \textit{script}, ele possui um grande poder de 
...
(\textit{Distributed File System}) da plataforma. Os arquivos são divididos em 
